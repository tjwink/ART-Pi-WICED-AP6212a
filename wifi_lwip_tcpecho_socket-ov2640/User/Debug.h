...
#define YesOrNo(x) (x ? "YES" : "NO")
   ...
   extern mico_mutex_t stdio_tx_mutex1;


    #define custom_log(N, M, ...) do {if (mico_debug_enabled==0)break;\
                                      host_rtos_lock_mutex( &stdio_tx_mutex1 );\
                                      printf("[%d][%s: %s:%4d] " M "\r\n", host_rtos_get_time(), N, SHORT_FILE, __LINE__, ##__VA_ARGS__);\
                                      host_rtos_unlock_mutex( &stdio_tx_mutex1 );}while(0==1)
                                        
    #define debug_print_assert(A,B,C,D,E,F) do {if (mico_debug_enabled==0)break;\
                                                     host_rtos_lock_mutex( &stdio_tx_mutex1 );\
                                                     printf("[%d][MICO:%s:%s:%4d] **ASSERT** %s""\r\n", host_rtos_get_time(), D, F, E, (C!=NULL) ? C : "" );\
                                                     host_rtos_unlock_mutex( &stdio_tx_mutex1 );}while(0==1)
    #if TRACE
...
*/
    ...
    #define require_noerr_string( ERR,  STR )                                                         \
        do                                                                                                  \
        {                                                                                                   \
            OSStatus        localErr;                                                                       \
                                                                                                            \
