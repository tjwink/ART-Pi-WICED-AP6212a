...
/**
 *结构的最大尺寸 (i.e. largest bus implementation)
 */
 ...
 */
#define WICED_LINK_OVERHEAD_BELOW_ETHERNET_FRAME_MAX ( MAX_BUS_HEADER_LENGTH + MAX_SDPCM_HEADER_LENGTH )//12+18
 ...
 * The size in bytes of the Link layer header i.e. the Wiced specific headers and the Ethernet header
 以字节为单位的链路层报头的大小，即所连接的特定报头和以太网报头
 */
#define WICED_PHYSICAL_HEADER       (WICED_LINK_OVERHEAD_BELOW_ETHERNET_FRAME_MAX + WICED_ETHERNET_SIZE)//12+18+14
 ...
 */
#define WICED_WIFI_BUFFER_SIZE            (WICED_PAYLOAD_MTU + WICED_PHYSICAL_HEADER + WICED_PHYSICAL_TRAILER)//1500+（12+18+14）+0
