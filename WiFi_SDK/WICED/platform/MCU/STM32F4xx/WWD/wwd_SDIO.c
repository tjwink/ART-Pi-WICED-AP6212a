...
ALIGNED_PRE(4) static uint8_t       temp_dma_buffer[MAX(2*1024,WICED_LINK_MTU+ 64)] ALIGNED(4);//DMA缓存数组
static uint8_t*                     user_data;
static uint32_t                     user_data_size;
static uint8_t*                     dma_data_source;
static uint32_t                     dma_transfer_size;
static host_semaphore_type_t        sdio_transfer_finished_semaphore;//sdio传输完成的信号量
...
wwd_result_t host_platform_sdio_transfer( wwd_bus_transfer_direction_t direction, wwd_sdio_command_t command, sdio_transfer_mode_t mode, sdio_block_size_t block_size, uint32_t argument, /*@null@*/ uint32_t* data, uint16_t data_size, sdio_response_needed_t response_expected, /*@out@*/ /*@null@*/ uint32_t* response )
{
...
void SDIO_IRQHandler(void)
{
  /* Process All SDIO Interrupt Sources */
	//printf("SDIO_IRQHandler -> 41\r\n");
  sdio_irq();
}
//ulCurrentInterrupt = 4b 
void DMA2_Stream3_IRQHandler(void)
{
//	printf("DMA2_Stream3_IRQHandler -> 4b\r\n");
  sdio_dma_irq();
}
...
//////            else if (current_command == SDIO_CMD_53)
//////            {
////////                if (current_transfer_direction == BUS_WRITE)//DMA_Priority_Medium
////////                {
////////                    DMA2_Stream3->CR = DMA_DIR_MemoryToPeripheral |
////////                                       DMA_Channel_4 | DMA_PeripheralInc_Disable | DMA_MemoryInc_Enable |
////////                                       DMA_PeripheralDataSize_Word | DMA_MemoryDataSize_Word |
...
//////                                       DMA_Channel_4 | DMA_PeripheralInc_Disable | DMA_MemoryInc_Enable |
//////                                       DMA_PeripheralDataSize_Word | DMA_MemoryDataSize_Word |
//////                                       DMA_Mode_Normal | DMA_Priority_Low |
//////                                       DMA_MemoryBurst_INC4 | DMA_PeripheralBurst_INC4 | DMA_SxCR_PFCTRL | DMA_SxCR_EN | DMA_SxCR_TCIE;
//////                }
//////                else
//////                {
