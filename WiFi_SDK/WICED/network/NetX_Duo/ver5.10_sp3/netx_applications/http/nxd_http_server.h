...
#define NX_HTTP_MAX_STRING                  (NX_HTTP_MAX_NAME + NX_HTTP_MAX_PASSWORD + (NX_HTTP_MAX_NAME + NX_HTTP_MAX_PASSWORD)/2)
...
typedef struct NX_HTTP_SERVER_MULTIPART_STRUCT
{
    ...
    /* Boundary string.  */
    UCHAR           nx_http_server_multipart_boundary[NX_HTTP_MAX_HEADER_FIELD];
...
VOID        _nx_http_server_response_send(NX_HTTP_SERVER *server_ptr, CHAR *status_code, CHAR *information, CHAR *additional_info);
UINT        _nx_http_server_basic_authenticate(NX_HTTP_SERVER *server_ptr, NX_PACKET *packet_ptr, CHAR *name_ptr, CHAR *password_ptr, CHAR *realm_ptr, UINT *auth_request_present);
...
#ifdef  NX_HTTP_DIGEST_ENABLE
UINT        _nx_http_server_digest_authenticate(NX_HTTP_SERVER *server_ptr, NX_PACKET *packet_ptr, CHAR *name_ptr, CHAR *password_ptr, CHAR *realm_ptr, UINT *auth_request_present);
...
UINT        _nx_http_server_match_string(UCHAR *src_start, UCHAR *src_end, UCHAR *target, ULONG *match_count, UCHAR **match_end_ptr);
UINT        _nx_http_server_field_value_get(NX_PACKET *packet_ptr, UCHAR *field_name, UCHAR *field_value, ULONG field_value_size);
UINT        _nx_http_server_memicmp(UCHAR *src, ULONG src_length, UCHAR *dest, ULONG dest_length);

UINT        _nx_http_server_generate_response_header(NX_HTTP_SERVER *server_ptr, NX_PACKET **packet_pptr, 
                                                     CHAR *status_code, UINT content_length, CHAR *content_type, CHAR* additional_header);
