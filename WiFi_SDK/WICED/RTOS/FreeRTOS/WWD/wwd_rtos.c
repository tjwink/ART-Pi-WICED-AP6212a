...
设置一个信号量
*如果有任何线程正在等待信号量，则会恢复第一个线程
*否则增加信号量。
*可以从中断上下文调用，因此必须能够处理恢复其他
*来自中断上下文的线程。
 * @param semaphore       : 指向将接收已创建信号量句柄的变量的指针
 * @param called_from_ISR : wiced_true的值表示从中断上下文调用
 *                          wiced_false的值表示从正常线程上下文调用
 *
 * @return wwd_result_t : 如果成功设置了信号量，则返回成功
 *                        : 如果发生错误，则返回wiced_error
...
wwd_result_t host_rtos_set_semaphore( host_semaphore_type_t* semaphore, wiced_bool_t called_from_ISR )
{
    ...
    if ( called_from_ISR == WICED_TRUE )//如果为wiced_ture 说明从中断内 使用
    {
				//获取一个计数信号量（中断）
        signed portBASE_TYPE xHigherPriorityTaskWoken;
        result = xSemaphoreGiveFromISR( *semaphore, &xHigherPriorityTaskWoken );//在中断内释放信号量
    ...
    {//获取一个计数信号量
        result = xSemaphoreGive( *semaphore );//给出二值信号量
        wiced_assert( "Unable to set semaphore", result == pdTRUE );
    }
