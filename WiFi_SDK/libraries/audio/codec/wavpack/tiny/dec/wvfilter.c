...
static int32_t read_bytes (void *buff, void* wv_id, int32_t bcount);
...
int main ()
{
    ...
    wpc = WavpackDecOpenFileInput (read_bytes, NULL, error);
    ...
    num_channels = WavpackDecGetReducedChannels (wpc);
    total_samples = WavpackDecGetNumSamples (wpc);
    bps = WavpackDecGetBytesPerSample (wpc);
    ...
    while (1) {
        ...
        samples_unpacked = WavpackDecUnpackSamples (wpc, temp_buffer, 256 / num_channels);
    ...
    if (WavpackDecGetNumSamples (wpc) != (uint32_t) -1 &&
        total_unpacked_samples != WavpackDecGetNumSamples (wpc)) {
            fputs ("incorrect number of samples!\n", stderr);
            return 1;
    }

    if (WavpackDecGetNumErrors (wpc)) {
