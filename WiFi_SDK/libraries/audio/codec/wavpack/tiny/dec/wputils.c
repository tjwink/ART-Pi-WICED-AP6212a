...
const uint32_t dec_sample_rates [] = { 6000, 8000, 9600, 11025, 12000, 16000, 22050,
    24000, 32000, 44100, 48000, 64000, 88200, 96000, 192000 };

///////////////////////////// executable code ////////////////////////////////

static uint32_t read_next_header (read_stream infile, void* wv_in, WavpackDecHeader *wphdr);
...
WavpackDecContext *WavpackDecOpenFileInput (read_stream infile, void *wv_id, char *error)
{
    ...
    while (!wps->wphdr.block_samples) {
        ...
        bcount = read_next_header (wpc.infile, wpc.wv_in, &wps->wphdr);
    ...
    if (!wpc.config.sample_rate) {
        if (!wps || !wps->wphdr.block_samples || (wps->wphdr.flags & SRATE_MASK) == SRATE_MASK)
            ...
            wpc.config.sample_rate = dec_sample_rates [(wps->wphdr.flags & SRATE_MASK) >> SRATE_LSB];
    }
...
static uint32_t read_next_header (read_stream infile, void* wv_in, WavpackDecHeader *wphdr)
{
    ...
    while (1) {
        ...
        if (infile (wv_in, buffer + bleft, sizeof (*wphdr) - bleft) != (int32_t) sizeof (*wphdr) - bleft)
            return -1;
        ...
        if (*sp++ == 'w' && *sp == 'v' && *++sp == 'p' && *++sp == 'k' &&
            ...
            sp [4] >= (MIN_STREAM_VERS & 0xff) && sp [4] <= (MAX_STREAM_VERS & 0xff)) {
                ...
                little_endian_to_native (wphdr, WavpackDecHeaderFormat);
