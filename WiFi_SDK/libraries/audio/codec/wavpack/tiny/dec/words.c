...
#define count_bits(av) ( \
 (av) < (1 << 8) ? dec_nbits_table [av] : \
  ( \
   (av) < (1L << 16) ? dec_nbits_table [(av) >> 8] + 8 : \
   ((av) < (1L << 24) ? dec_nbits_table [(av) >> 16] + 16 : dec_nbits_table [(av) >> 24] + 24) \
  ) \
)

///////////////////////////// local table storage ////////////////////////////

const char dec_nbits_table [] = {
...
void init_words_dec (WavpackDecStream *wps)
{
    CLEAR (wps->w);
}

static int mylog2_dec (uint32_t avalue);
...
int read_entropy_vars_dec (WavpackDecStream *wps, WavpackDecMetadata *wpmd)
{
    ...
    wps->w.c [0].median [0] = exp2s_dec (byteptr [0] + (byteptr [1] << 8));
    wps->w.c [0].median [1] = exp2s_dec (byteptr [2] + (byteptr [3] << 8));
    wps->w.c [0].median [2] = exp2s_dec (byteptr [4] + (byteptr [5] << 8));

    if (!(wps->wphdr.flags & MONO_DATA)) {
        wps->w.c [1].median [0] = exp2s_dec (byteptr [6] + (byteptr [7] << 8));
        wps->w.c [1].median [1] = exp2s_dec (byteptr [8] + (byteptr [9] << 8));
        wps->w.c [1].median [2] = exp2s_dec (byteptr [10] + (byteptr [11] << 8));
    }
...
int read_hybrid_profile_dec (WavpackDecStream *wps, WavpackDecMetadata *wpmd)
{
    ...
    if (wps->wphdr.flags & HYBRID_BITRATE) {
        wps->w.c [0].slow_level = exp2s_dec (byteptr [0] + (byteptr [1] << 8));
        byteptr += 2;

        if (!(wps->wphdr.flags & MONO_DATA)) {
            wps->w.c [1].slow_level = exp2s_dec (byteptr [0] + (byteptr [1] << 8));
            byteptr += 2;
        }
    }
    ...
    if (byteptr < endptr) {
        wps->w.bitrate_delta [0] = exp2s_dec ((short)(byteptr [0] + (byteptr [1] << 8)));
        byteptr += 2;

        if (!(wps->wphdr.flags & MONO_DATA)) {
            wps->w.bitrate_delta [1] = exp2s_dec ((short)(byteptr [0] + (byteptr [1] << 8)));
            byteptr += 2;
        }
...
void update_error_limit_dec (struct words_data *w, uint32_t flags)
{
    ...
    if (flags & MONO_DATA) {
        if (flags & HYBRID_BITRATE) {
            ...
            if (slow_log_0 - bitrate_0 > -0x100)
                w->c [0].error_limit = exp2s_dec (slow_log_0 - bitrate_0 + 0x100);
            else
                w->c [0].error_limit = 0;
        }
        else
            w->c [0].error_limit = exp2s_dec (bitrate_0);
    }
    else {
        ...
        if (flags & HYBRID_BITRATE) {
            ...
            if (slow_log_1 - bitrate_1 > -0x100)
                w->c [1].error_limit = exp2s_dec (slow_log_1 - bitrate_1 + 0x100);
            else
                w->c [1].error_limit = 0;
        }
        else {
            w->c [0].error_limit = exp2s_dec (bitrate_0);
            w->c [1].error_limit = exp2s_dec (bitrate_1);
        }
    }
}

static uint32_t read_code_dec (Bitstream *bs, uint32_t maxcode);
                ...
                struct words_data *w, Bitstream *bs)
{
    ...
    for (csamples = 0; csamples < nsamples; ++csamples) {
        ...
        if ((flags & HYBRID_FLAG) && ((flags & MONO_DATA) || !(csamples & 1)))
            update_error_limit_dec (w, flags);
        ...
        if (!c->error_limit)
            mid = read_code_dec (bs, high - low) + low;
        ...
        *buffer++ = getbit (bs) ? ~mid : mid;
        ...
        if (flags & HYBRID_BITRATE)
            c->slow_level = c->slow_level - ((c->slow_level + SLO) >> SLS) + mylog2_dec (mid);
    }
...
static int mylog2_dec (uint32_t avalue)
{
    ...
    if ((avalue += avalue >> 9) < (1 << 8)) {
        dbits = dec_nbits_table [avalue];
        return (dbits << 8) + log2_table [(avalue << (9 - dbits)) & 0xff];
    }
    else {
        if (avalue < (1L << 16))
            dbits = dec_nbits_table [avalue >> 8] + 8;
        else if (avalue < (1L << 24))
            dbits = dec_nbits_table [avalue >> 16] + 16;
        else
            dbits = dec_nbits_table [avalue >> 24] + 24;
...
int log2s_dec (int32_t value)
{
    return (value < 0) ? -mylog2_dec (-value) : mylog2_dec (value);
}
