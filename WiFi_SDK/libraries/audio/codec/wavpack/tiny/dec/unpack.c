...
int init_wv_bitstream (WavpackDecContext *wpc, WavpackDecMetadata *wpmd)
{
    ...
    if (wpmd->data)
        bs_open_read (&wps->wvbits, wpmd->data, (unsigned char *) wpmd->data + wpmd->byte_length, NULL, wpc->wv_in, 0);
    else if (wpmd->byte_length)
        bs_open_read (&wps->wvbits, wpc->read_buffer, wpc->read_buffer + sizeof (wpc->read_buffer),
                      wpc->infile, wpc->wv_in, wpmd->byte_length + (wpmd->byte_length & 1));
...
int read_decorr_weights (WavpackDecStream *wps, WavpackDecMetadata *wpmd)
{
    ...
    while (--dpp >= wps->decorr_passes && termcnt--) {
        dpp->weight_A = restore_weight_dec (*byteptr++);

        if (!(wps->wphdr.flags & MONO_DATA))
            dpp->weight_B = restore_weight_dec (*byteptr++);
    }
...
int read_decorr_samples (WavpackDecStream *wps, WavpackDecMetadata *wpmd)
{
    ...
    while (dpp-- > wps->decorr_passes && byteptr < endptr)
        if (dpp->term > MAX_TERM) {
            dpp->samples_A [0] = exp2s_dec ((short)(byteptr [0] + (byteptr [1] << 8)));
            dpp->samples_A [1] = exp2s_dec ((short)(byteptr [2] + (byteptr [3] << 8)));
            byteptr += 4;

            if (!(wps->wphdr.flags & MONO_DATA)) {
                dpp->samples_B [0] = exp2s_dec ((short)(byteptr [0] + (byteptr [1] << 8)));
                dpp->samples_B [1] = exp2s_dec ((short)(byteptr [2] + (byteptr [3] << 8)));
                byteptr += 4;
            }
        }
        else if (dpp->term < 0) {
            dpp->samples_A [0] = exp2s_dec ((short)(byteptr [0] + (byteptr [1] << 8)));
            dpp->samples_B [0] = exp2s_dec ((short)(byteptr [2] + (byteptr [3] << 8)));
            byteptr += 4;
        }
        else {
            ...
            while (cnt--) {
                dpp->samples_A [m] = exp2s_dec ((short)(byteptr [0] + (byteptr [1] << 8)));
                byteptr += 2;

                if (!(wps->wphdr.flags & MONO_DATA)) {
                    dpp->samples_B [m] = exp2s_dec ((short)(byteptr [0] + (byteptr [1] << 8)));
                    byteptr += 2;
                }
...
static void fixup_samples (WavpackDecStream *wps, int32_t *buffer, uint32_t sample_count);
...
int32_t unpack_samples (WavpackDecContext *wpc, int32_t *buffer, uint32_t sample_count)
{
    ...
    if (flags & MONO_DATA) {
        ...
        i = get_words_dec (buffer, sample_count, flags, &wps->w, &wps->wvbits);
