...
static char g_render_device_udn[RENDER_DEVICE_UDN_SIZE];
			 ...
			 const char *uuid)
{
    ...
    rc = snprintf(g_render_device_udn, sizeof(g_render_device_udn), "uuid:%s", uuid);
    if ( rc > 0 )
    {
        if (rc >= sizeof(g_render_device_udn))
        {
            Log_error("renderer", "UUID was truncated !");
        }
        render_device.udn = g_render_device_udn;
    }
