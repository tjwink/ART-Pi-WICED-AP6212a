...
static wiced_bool_t initialize_device(struct upnp_device_descriptor *device_def,
				  ...
				  unsigned short port)
{
	...
	rc = UpnpInit(iface, ip_address, port);
...
static int webserver_register_resource(struct icon *icon, const char *content_type)
{
    int               rc            = -1;
    uint32_t          resource_size = 0;
    resource_result_t result;

    if ( (icon->resource == NULL) || (icon->resource_buffer != NULL) )
    {
        Log_error("upnp", "webserver_register_resource(): resource is NULL or resource_buffer is not NULL !");
        goto _exit;
    }

    icon->resource_buffer = malloc(icon->resource->size);
    if ( icon->resource_buffer == NULL )
    {
        Log_error("upnp", "webserver_register_resource(): malloc() failed !");
        goto _exit;
    }

    result = resource_read( icon->resource, 0, icon->resource->size, &resource_size, icon->resource_buffer );
    if ( (result != RESOURCE_SUCCESS) || (resource_size < icon->resource->size) )
    {
        Log_error("upnp", "webserver_register_resource(): resource_read() failed !");
        goto _exit;
    }

    rc = webserver_register_data(icon->url, (char *)icon->resource_buffer, (size_t)icon->resource->size, content_type);
    if ( rc != 0 )
    {
        Log_error("upnp", "webserver_register_resource(): webserver_register_data() failed !");
    }
				     ...
				     unsigned short port)
{
	...
	for (int i = 0; (icon_entry = device_def->icons[i]); i++) {
	    webserver_register_resource(icon_entry, "image/png");
	}
	...
	if (!initialize_device(device_def, result_device, iface, ip_address, port)) {
	    UpnpFinish();
	    free(result_device);
	    return NULL;
	}
...
void upnp_device_shutdown(struct upnp_device *device) {
	...
	for (int i = 0; (icon_entry = device->upnp_device_descriptor->icons[i]); i++) {
	    icon_entry->resource_buffer = NULL;
	}
	webserver_cleanup();

	ithread_mutex_destroy(&(device->device_mutex));
	if ( device->upnp_device_descriptor->deinit_function != NULL )
	{
	    device->upnp_device_descriptor->deinit_function();
	}
	free(device);
}
