...
char *get_element_attribute(struct xmlelement *element, const char *attribute_name) {
    IXML_Element *elem = to_ielem(element);
    const char *attribute_value = (elem != NULL
                  ? ixmlElement_getAttribute(elem, attribute_name)
                  : NULL);
                            ...
                            const char *tagname, long long value)
{
        ...
        if (snprintf(buf, sizeof(buf), /* "%lld" */ "%ld", (long)value) >= 0) {
		add_value_element(doc, parent, tagname, buf);
	}
}
