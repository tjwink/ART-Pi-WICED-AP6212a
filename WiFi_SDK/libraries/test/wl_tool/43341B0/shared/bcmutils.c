 ...
 * $Id: bcmutils.c 374677 2012-12-14 02:01:01Z sgsiener $
 */
...
static char *
#if defined(WLTEST) || defined(BCMINTERNAL) || defined(BCMDBG_DUMP) || !defined(WLC_HIGH)
...
/* IPv4 style checksum calculation */
static uint32
checksum_ipv4_part(const uint8 *pb, int len)
{
	const uint16 *ps;
	uint16 t = 0;
	const uint32 *pl;
...
uint16
checksum_ipv4(void *dataptr, uint16 len)
{
	uint32 acc;

	acc = checksum_ipv4_part(dataptr, len);
	while (acc >> 16) {
...
#endif /* BCMDBG||BCMDBG_ERR||WLMSG_PRHDRS||WLMSG_PRPKT||WLMSG_ASSOC||BCMDBG_DUMP||DHD_DEBUG */
