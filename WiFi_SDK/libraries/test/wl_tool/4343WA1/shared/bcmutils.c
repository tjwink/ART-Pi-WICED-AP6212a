 ...
 * $Id: bcmutils.c 514055 2014-11-08 18:42:49Z sudhirbs $
 */
...
BCMATTACHFN(bcm_alloc_rxcplid_list)(osl_t *osh, uint32 rxcpl_max)
{
	...
	printf(rstr_allocating_rxcplid_buffers, rxcpl_max);
	...
	if (g_rxcplid_list == NULL) {
		printf(rstr_rxcplid_list_allocation_fail, size, rxcpl_max);
		return FALSE;
	}
...
pktpool_dbg_dump(pktpool_t *pktp)
{
	...
	for (i = 0; i < pktp->dbg_qlen; i++) {
		...
		printf("%d, p: 0x%p dur:%u us state:%d\n", i,
...
void print_pktpool_stats(pktpool_t *pktp)
{
	pktpool_stats_t pstats;

	pktpool_stats_dump(pktp, &pstats);
	printf("txdh:%02d txd11:%02d enq:%02d rxdh:%02d rxd11:%02d rxfill:%02d idle:%02d rxurb:%02d"
	       " idle:%02d other:%02d\n", pstats.txdh, pstats.txd11, pstats.enq, pstats.rxdh,
	       pstats.rxd11, pstats.rxfill, pstats.idle, pstats.rxurb, pstats.init, pstats.other);
}
...
pktpool_get(pktpool_t *pktp)
{
	...
	if (p == NULL) {
		...
		if (p == NULL) {
#ifdef BCM_DBG_POOL_PRINT_EMPTY
			pktp->pool_empty_cnt++;
			if ((pktp->pool_empty_cnt % 256 == 0) && (pktp->id == 1)) {
				print_pktpool_stats(pktp);
			}
#endif
			return NULL;
		}
