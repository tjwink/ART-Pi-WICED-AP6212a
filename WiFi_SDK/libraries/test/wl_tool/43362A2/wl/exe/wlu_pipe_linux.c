 ...
 *
 * $Id: wlu_pipe_linux.c,v 1.14.4.1 2010-12-14 05:41:24 jwang Exp $
 */
...
rwl_open_serial(int remote_type, char *port)
{
	...
	*bsdPath = '\0';
	/* try to find the adapter directly */
		...
		if (classesToMatch == NULL)
			...
			kernResult =
			        IOServiceGetMatchingServices(masterPort,
			                                     classesToMatch, &serialPortIterator);
	...
	/* O_NONBLOCK is needed just temporarily */
	/* actually open the port */
	fileDescriptor = open(bsdPath, O_RDWR | O_NOCTTY | O_NDELAY | O_SYNC | O_NONBLOCK);
	...
	/* original settings to be reset later */
	tcgetattr(fileDescriptor, &orig);
	...
	/* 115.2kbaud, 8 bit words, ignore parity, 1 stop bit - these shouldn't be hardcoded */
	/* for one stop bit, no need to get CSTOPB - one bit is assumed */
	/* set to a local device, enable the receiver) */
