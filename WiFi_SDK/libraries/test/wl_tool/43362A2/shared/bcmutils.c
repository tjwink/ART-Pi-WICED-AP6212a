 ...
 * $Id: bcmutils.c,v 1.277.2.18 2011-01-26 02:32:08 csm Exp $
 */
...
/* Strictly set maxlen irrespective of current pktpool_len. This is the
	difference for this function compared to pktpool_setmaxlen().
	Note that, the user of this function has to make sure that, wl is
	down. Also, note that, there is NO surety that required maxlen
	is achieved because of buffers allocated for wl/bus side.
	But it will reduce number of buffers in the pool.
 */
int
pktpool_setmaxlen_strict(osl_t *osh, pktpool_t *pktp, uint16 maxlen)
{
	int i;
	int cnt;
	if (maxlen > PKTPOOL_LEN_MAX)
	...
	/* if current maxlen [pktp->maxlen] was more than maxlen required,
		free up the remaining buffers from the end of the pool. ELSE, a
		subsequent pktpool_fill() should be called for filling up.
	 */
	if (pktp->len > maxlen) {
		/* num of pkts to free from pool */
		cnt = pktp->len - maxlen;
		for (i = (pktp->len - 1); i >= 0; i--) {
			if (pktp->q[i] != NULL) {
				PKTSETPOOL(osh, pktp->q[i], FALSE, NULL);
				PKTFREE(osh, pktp->q[i], pktp->istx);
				pktp->q[i] = NULL;
...
char *
pktlist_dump(osl_t *osh, pktlist_info_t *pktlist, char *buf)
{
	char *obuf = buf;
	uint i;
...
/* #ifdef NOTDEF  Remove this ifdef to print pkttag and pktdata */
		if (buf != NULL) {
			...
			while (pkt) {
				...
				npkt = (void *)PKTNEXT(osh, pkt);
				PKTSETNEXT(osh, pkt, NULL);

				src = (char *)(PKTTAG(pkt));
				pktlen = PKTLEN(NULL, pkt);
				hroom = PKTHEADROOM(osh, pkt);
...
#endif /* BCMDBG||WLMSG_PRHDRS||WLMSG_PRPKT||WLMSG_ASSOC||BCMDBG_DUMP||DHD_DEBUG||WLMEDIAPEAKRATE */
