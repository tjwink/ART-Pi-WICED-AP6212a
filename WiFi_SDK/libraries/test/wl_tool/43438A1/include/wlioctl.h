 ...
 *
 * $Id: wlioctl.h 491851 2014-07-17 21:03:39Z rvossen $
 */
...
typedef BWL_PRE_PACKED_STRUCT struct  dctl_prof {
	...
	/* modes to switch between */
	uint8	default_btc_mode;
	uint8	fallback_btc_mode;
	...
	/* simulated bt pwr and wl rssi : todo move to btch */
	int8 sim_btpwr;
	int8 sim_wlrssi;
...
enum {
	...
	WL_PROXD_FLAG_TX_CIVIC_LOC 	= 0x00000008, /* tx civic loc, if available */
...
/* lci request - need to move to 802.11.h */
enum wl_proxd_lci_subject {
	WL_PROXD_LCI_SUBJ_LOCAL = 0,
	WL_PROXD_LCI_SUBJ_REMOTE = 1,
	WL_PROXD_LCI_SUBJ_THIRD_PARTY = 2
};
typedef int8 wl_proxd_lci_subj_t;

typedef struct wl_proxd_lci_req {
	wl_proxd_lci_subj_t subj;
	uint8				pad[3];
	/* no options */
} wl_proxd_lci_req_t;

/* LCI definition RFC 6225 along w/ macros to access fields - 8-208 11mc2.6/2014 */
typedef struct wl_proxd_lci {
	uint8 data[16];
} wl_proxd_lci_t;
...
/* Civic location request - 8.4.2.20.14 11mc2.6/2014 */
typedef struct wl_proxd_civic_loc_req {
	uint8  subj;
	uint8  type;
	uint8  siu;	/* service interval units */
	uint8  pad;
	uint16 si;  /* service interval */
} wl_proxd_civic_loc_req_t;

/* civic location - RFC 4776 - civic location type starting w/ country field */
typedef struct wl_proxd_civic_loc {
	uint16 len;
	uint8 value[1]; /* variable */
} wl_proxd_civic_loc_t;

/* availability. advertising mechanism bss specific */
typedef struct wl_proxd_avail {
	uint16 tbtt_imax;		/* TBTT interval divided into slots; not all supported */
	uint8  pad[2];
	struct {
		uint16 tbtt_ioff;	/* interval number */
		uint8 pad[2];
		uint32 chan;	/* chanspec */
	} slots[1];			/* variable */
} wl_proxd_avail_t;
...
enum {
	...
	WL_PROXD_TLV_ID_BURST_NUM 		= 15, 	/* in a session */
	...
	WL_PROXD_TLV_ID_CIVIC_LOC_REQ 	= 22,
	WL_PROXD_TLV_ID_CIVIC_LOC 		= 23,
	WL_PROXD_TLV_ID_AVAIL			= 24,
	WL_PROXD_TLV_ID_SESSION_FLAGS	= 25,
	WL_PROXD_TLV_ID_SESSION_FLAGS_MASK	= 26,	/* in only */
	WL_PROXD_TLV_ID_BURST_TIMEOUT	= 27,	/* expect response within - intvl */
...
#define WL_PROXD_EVENT_MASK_ALL 0xffffffff
