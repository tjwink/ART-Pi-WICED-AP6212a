...
/** wiced audio routes supported in the embedded mode */
typedef enum
{
    WICED_BT_A2DP_ROUTE_I2S                  = 0x00,  /**< Route the PCM Samples over I2S. Read from I2S in case of audio source,write to I2S in case of audio sink */
    WICED_BT_A2DP_ROUTE_UART                 = 0x01,  /**< Route the PCM samples over transport. Receive the audio data to be sent OTA from transport in case of audio source.Receive the audio data OTA, decode and send to transport in case of audio sink */
    WICED_BT_A2DP_ROUTE_SINE                 = 0x02,  /**< Route the stored sine samples over the air. Applicable in case of audio source */
    WICED_BT_A2DP_ROUTE_APP                  = 0x03,  /**< Route the PCM samples to app.  Receive the audio data OTA, decode and send to app. Applicable in case of audio sink */
    WICED_BT_A2DP_ROUTE_COMPRESSED_TRANSPORT = 0x04,  /**< Route the compressed audio data(AVDTP media packet, including the header) over transport. Receive the audio data OTA and send to transport. Applicable in case of audio sink */
    WICED_BT_A2DP_ROUTE_COMPRESSED_APP       = 0x05,  /**< Route the compressed audio data(AVDTP media packet, including the header) to app. Receive the audio data OTA and send to transport. Applicable in case of audio sink */
}wiced_bt_a2dp_route_t;
...
#ifdef USE_WICED_HCI
/** API to change the audio route used for streaming
 *
 *  Called by the application when it wants to update the audio route. New route shall be
 *  used in the next streaming session.
 *
 *  @param bd_address    Bluetooth device address of the connected peer device
 *
 *  @param route    Route to be used for streaming.
 *
 *  @return wiced_result_t (WICED_BT_XXX)
 */
