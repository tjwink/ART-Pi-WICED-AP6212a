...
while(@ARGV && !$done_popping_opts)
{
    ...
    elsif( $ARGV[0] eq '-fm' || $ARGV[0] eq '--firmware' )
    {
        shift @ARGV;
        $app_name = $ARGV[0];
        shift @ARGV;
...
if($help)
{
    printf( "\n".
            ...
            "       -s  | --nohciheader               ".
                                        "excludes hci header\n".
            "       -fm | --firmware                  ".
            "        Usage: hcd2c.pl -fm app_name INFILE ".
            "                          generates complete firmware file.c\n".
            "\n" );
    exit -1;
}
...
{
    $outFirmFileName  = "bt_firmware_embedded_$app_name".".c";
    open( $fh, '>', $outFirmFileName ) || die;
    binmode $fh;

    printf $fh "\/*\n";
    printf $fh " * \$ Copyright Cypress Semiconductor \$\n";
    printf $fh " *\/\n";
    printf $fh "#include <stdint.h>\n";
    printf $fh "\n";
    printf $fh "\/* labelling: appname-(chipname)(stepping)-frequency-(headset GIT SHA)-(source)-Wiced-release.hcd *\/\n";
    printf $fh "\n";
    printf $fh "\/*Note: GIT_SHA will be having first 8 digits of the commit_id from 20706A2 source *\/\n";
    printf $fh "\n";
    printf $fh "const char brcm_patch_version[] = \"headset-BCM920706A2-40MHz-xxxxxxxx-src-Wiced-release.hcd\"\;\n";
    printf $fh "\n";
    printf $fh "const uint8_t brcm_patchram_format = 0x01;\n";
    printf $fh "\n";
    printf $fh "const uint8_t brcm_patchram_buf[] =\n";
    printf $fh "{\n";
    printf $fh "\/* Configuration Data Records (Write_RAM) *\/\n";
}
else
{
...
while(!$done)
{
    ...
    else
    {
        ...
        if( $generate_firmware )
        {
            printf $fh "0x4C, 0xFC, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X,",
                $data_length+4,
                ($addr)       & 0xFF,
                ($addr >> 8)  & 0xFF,
                ($addr >> 16) & 0xFF,
...
if( $generate_firmware )
{
    ...
    printf $fh "0x4E, 0xFC, 0x04, 0xFF, 0xFF, 0xFF, 0xFF\n";
}
elsif ($no_hci_header eq 0)
{
    printf("\n/* Commit configuration data, reboot firmware (Launch_RAM) */\n");
...
if( $generate_firmware )
{
    printf $fh "};\n";
    printf $fh "\n";
    printf $fh "const int brcm_patch_ram_length = sizeof(brcm_patchram_buf);\n";
    close $fh;
}
else
{
