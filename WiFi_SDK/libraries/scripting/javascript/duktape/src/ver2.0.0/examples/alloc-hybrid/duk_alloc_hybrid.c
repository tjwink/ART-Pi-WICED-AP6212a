...
#ifdef WICED_USE_DDR_HEAP
    st = (pool_state *) platform_heap_malloc(PLATFORM_HEAP_DDR, sizeof(pool_state));
#else
...
#ifdef WICED_USE_DDR_HEAP
    st->alloc_start = (char *) platform_heap_malloc(PLATFORM_HEAP_DDR, total_size);
#else
	st->alloc_start = (char *) malloc(total_size);
#endif
	if (!st->alloc_start) {
#ifdef WICED_USE_DDR_HEAP
        platform_heap_free(PLATFORM_HEAP_DDR, st);
#else
		free(st);
#endif
		return NULL;
...
#ifdef WICED_USE_DDR_HEAP
        platform_heap_free(PLATFORM_HEAP_DDR, st->alloc_start);
#else
		free(st->alloc_start);
#endif
	}
...
#endif
#ifdef WICED_USE_DDR_HEAP
        return platform_heap_malloc(PLATFORM_HEAP_DDR, size);
#else
		return malloc(size);
#endif
	}
...
void *duk_realloc_hybrid(void *udata, void *ptr, duk_size_t size) {
	...
	if (ADDR_IN_STATE_ALLOC(st, ptr)) {
		...
		if (size == 0) {
#ifdef WICED_USE_DDR_HEAP
            platform_heap_free(PLATFORM_HEAP_DDR, ptr);
#else
			free(ptr);
#endif
			return NULL;
...
#endif
#ifdef WICED_USE_DDR_HEAP
            return platform_heap_realloc(PLATFORM_HEAP_DDR, ptr, size);
#else
