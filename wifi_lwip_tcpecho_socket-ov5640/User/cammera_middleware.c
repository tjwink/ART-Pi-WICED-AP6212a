...
void DCMI_Start(void)
{
	...
	/*获取写缓冲区指针，准备写入新数据*/
	data_p = cbWrite(&cam_circular_buff);//在这获取的指针地址不对
	
	//printf("data_p=0x%x,data_p->head=0x%x\r\n",(int )data_p,(int )(data_p->head));
	
	if (data_p != NULL)	//若缓冲队列晰，开始传输
	{		
		...
		//printf("0x%x\r\n",(int )(data_p->head));
		//HAL_DCMI_Start_DMA(&DCMI_Handle, DCMI_MODE_CONTINUOUS, (uint32_t )LCD_FB_START_ADDRESS, CAMERA_QUEUE_DATA_LEN);
